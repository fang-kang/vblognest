...
import { CategoryBaseDto, CategoryDelDto, CategoryEditDto, CategoryListDto } from './dto/category.dto';
...
export class CategoryService {
  ...
  */
  async getCategoryCount(): Promise<number> {
    return await this.categoryRepository.createQueryBuilder('category').getCount()
  }
  /*
  *@Description: 获取分类列表
  *@MethodAuthor: fk
 ...
 async getCategoryList(params: CategoryListDto): Promise<CategoryInterface[]> {
  ...
  async updateCategory(params: CategoryEditDto): Promise<any> {
    let data = await this.categoryRepository.findOne(params.id);
    if(!data){
  ...
  async deleteCategory(params :CategoryDelDto): Promise<any> {
    ...
    return await this.categoryRepository.remove(data).then(()=>{
      return '删除成功'
    }).catch(err=>{
      console.log(err)
      throw new CustomException('操作失败')
